...
class WechatService extends Service
{
    ...
    public static function __callStatic(string $name, array $arguments)
    {
        ...
        if (getSite('wechatcfg.type') === 'api' || in_array($type, ['WePay', 'WePayV3'])) {
            if (class_exists($class)) {
                return new $class($type === 'WeMini' ? static::getWxconf() : static::getConfig());
            }
            throw new Exception("抱歉，接口模式无法实例 {$class} 对象！");
        } else {
            [$appid, $appkey] = [getSite('wechatcfg.thr_appid'), getSite('wechatcfg.thr_appkey')];
...
//            $jsonrpc = url('service/api.client/jsonrpc',[],false,true) . '?token=TOKEN';
//            return new JsonRpcClient(str_replace('token=TOKEN', "token={$token}", $jsonrpc));

            // 修改第三方授权 修改本地系统授权
            $auth = ServiceAuth::mk()->where(['authorizer_appid' => $appid])->findOrEmpty();
            $auth->where(['id' => $auth->getAttr('id')])->inc('total')->update([]);

            return AuthService::__callStatic($name, [$appid]);
        }
    }
    ...
    public static function getAppid(): string
    {
        if (static::getType() === 'api') {
            return  (string)AdminService::getSite('wechatcfg.appid','');
        }
        return  (string)AdminService::getSite('wechatcfg.thr_appid','');
    }
    ...
    {
        $type = strtolower((string)AdminService::getSite('wechatcfg.type',''));
...
//            $wxapp = AdminService::getSite('wechatcfg');
            $config = [
                'appid' => $wxapp['thr_appid'] ?? '',
                'appsecret' => $wxapp['thr_appkey'] ?? '',
                'cache_path' => syspath('runtime/wechat'),
            ];
        }
